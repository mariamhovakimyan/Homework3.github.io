...
app.layout=html.Div([
	...
	html.Div([html.H1(children='Homework 3', style={"color":"rgb(102, 0, 0)", "text-align":"center", "font-weight":"bold",})], className="row"),
	...
	#Part2
		...
		html.Div([
			html.Div([html.P("Graph 2", style={"font-weight":"bold",}), html.P("One the right hand side we have US GDP graphed over time. The data is sourced from QUANDL API (FRED/GDP). Your task is to recreate exactly the same graph."),
			], className="four columns"),
		...
		html.Div([
			html.Div([html.P("Graph 5", style={"font-weight":"bold",}), html.P("Last graph is based on manually inputted data. It shows the Roadmap developed by an artificial startup. Task 1 is assumed to take the whole Janduary, while Task 2 is starting from March and ending in mid April. Afterwards, Task 3 begins and ends in the end of September. Recreate a similar Roadmap"),
			], className="four columns"),
